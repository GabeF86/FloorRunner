...
- **call_patterns are the single source of structure:** `scheduling.call_patterns` (CallPatternDoc jsonb, one active per site) defines how schedules are BUILT — weekend/block chains, post/pre-call fills, post-call day-off blocks, spans, placement passes, relief config. Never re-hardcode structure in the engine; extend the pattern schema instead (`src/lib/rulesEngine/callPattern.ts`). An invalid doc does NOT error — the engine silently falls back to `CLASSIC_PATTERN`, so validate before writing one.
- **The configurable rule_definitions / rule_sets feature was REMOVED (2026-09-15).** It was validation-only, never consulted during generation, and every definition had been inactive since it shipped. Validation now runs entirely on always-on evaluators in `evaluators.ts` — `eligibility`, `timeOff`, `weekendAdjacentPto`, `shiftSkills`, `poolEligibility`, `providerLimits`, `crossSite`, `scenarioProhibition`, and the defaults inside `coverage` / `openSlot`. There is no way to switch a check off, which is deliberate: the things worth checking here are clinical, not policy. A new check belongs in that file, driven by a column on `shift_types`, not by a rule row.
- **`/rules` is now the Scheduling Logic page** — a read view of the live generation contract (`src/lib/schedulingLogic.ts` renders it in English from the pattern doc + shift types), plus a plain-English editor that proposes NAMED edits (`patternEdit.ts`), validates them against the schema, and shows an English diff before writing.
