...
- [x] **Step 1: Identify the live project** — read `.env.local` (grep `SUPABASE_URL`); compare its project ref against `mcp__supabase__get_project_url` and `mcp__supabase-chiefos__get_project_url`. Apply patch18 via the MATCHING server's `apply_migration` (name `patch18_call_patterns`). If neither matches or `.env.local` is absent, DO NOT APPLY — leave the SQL file with a README note in the final summary. *(No MCP server matched; Gabriel applied the SQL manually via the dashboard editor on 2026-07-12.)*
- [x] **Step 2: Post-apply spot checks** — run 2026-07-12 via the app's service-role connection: 11 sites = 11 active seeded patterns; shift_types backfill sane (C1/C2/C3 call_rank 0/1/2, D4–D9 relief_rank 1–6, D-codes engine=call, day codes engine=day_pool, CC1/CC2 category-call); `historical_call_counts` RPC returns 61 rows with the expected shape; `assistant_actions` reachable; end-to-end: GET /api/scheduling/call-patterns serves the seeded classic pattern (200).
...
- [x] **Step 5: Merge** — `git checkout main && git merge --no-ff scheduling-v2 -m "Scheduling v2: data-driven call patterns, engine hardening, Claude assistant"`. Do not push (no remote workflow established). *(Merged 2026-07-10 as 0b6a53b after the full-branch review's three blockers — quota-relaxation re-gate (invariant 2), pre-patch18 degraded-mode coherence in sequenceAutoFill/dayShiftAutoGen — were fixed in 44e09f9 and independently re-verified to MERGE. 425 tests green on main.)*
