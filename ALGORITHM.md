...
**What comes back** (`GenerationResult`): `assignments` (with placement `source` + scoring `explanation`), `unfilled` (per-slot candidate rejections, trimmed to 3 + `omittedCandidates` by [`trimUnfilled`](src/lib/rulesEngine/trimUnfilled.ts)), `skippedDerived` (§9), `warnings` (load-time advisories, §15, + contradictory-request advisories, §11), `requestGrants` / `callRequestGrants` (no-call and call-request grant reports, §11), `workDayReport` (per-provider working-days accounting, §4.6), `optimizeStats` (§11), `metrics`/`seedMetrics`, `perf`.
...
```
par      = site.call_par_level                     # AUTHORITATIVE (2026-07-24) — never clamped to the pool
...
**Par-authoritative (Gabriel 2026-07-24 — SUPERSEDES the 2026-07-16 clamp)**: the stored `call_par_level` is THE denominator, in both directions, unconditionally ("I want the math to use a par of 11 and even though there are only 8.75 FTE thats fine"). A par above the pool's summed FTE makes Σ targets = bucket_total × poolFte/par < bucket_total — that gap is now DELIBERATE: obligations under-cover the schedule and the uncovered remainder is the **paid-pickup layer**, taken after the schedule is built (a call past someone's rounded obligation gets the OVER treatment and is paid extra; worked example: 42 weekday C1s ÷ 11 × 1.0 FTE = 3.82 → obligation 4, assignment #5 is a paid extra). Fill-all mode still fills every slot it can — the relaxation/mop-up passes cover the shrunken targets ("quota never blocks fills"); obligatory mode leaves the remainder open as ordinary `'obligation-cap'` slots. A par below pool FTE remains the legitimate spread-thinner choice it always was. The load-time coverage warning still fires when stored-par targets can't sum to a bucket's slot count — it now names the pickup layer instead of calling the par stale. The old `effectiveParLevel`/`clampParToPoolFte` helpers are deleted.

**At-least-one floor (2026-07-16)**: the saturday/sunday bucket split (patch24) shrank weekend buckets to a handful of slots per block, driving per-provider targets below 1 — and the strict check then gave *every* provider zero capacity in those buckets. (`Sat+Sun` used to share a merged bucket precisely to keep targets above 1; the split is the right fairness mechanism, so the floor now provides that guarantee instead.) Every positive-FTE provider's target is floored to 1 per bucket; fairness *ordering* is untouched — `scoreCall`'s lifetime-ratio sort still decides who actually gets the slot. The floor lives in genContext target computation, NOT in the eligibility gate — parity fixtures hand-build their target maps and stay meaningful.
...
The generate route accepts `{ fillMode: 'all' | 'obligatory' | 'weekend-only' }` (body, default `'all'`; unknown values degrade to `'all'` via `resolveFillMode`; UI: the fill-mode select beside Auto-Generate, persisted in localStorage).

**Whole-number obligations (accounting):** each provider's obligatory call count = `round(totalCallSlots / par × FTE)` — TOTAL level, round-half-up (1.5→2, 1.3→1, 0.45→0), where `par` is the stored `call_par_level` verbatim (**par-authoritative**, Gabriel 2026-07-24 — see §4; when pool ΣFTE < par, Σ obligations deliberately under-cover the block and the remainder is the paid-pickup layer, surfaced on the Call Counts modal's coverage line). Single-homed: rounding + extra math in [`src/lib/fteTarget.ts`](src/lib/fteTarget.ts) (`roundedObligation`, `extraCalls`, `selectOverParAssignmentIds`), engine census in [`obligation.ts`](src/lib/rulesEngine/obligation.ts) (`computeObligations`), UI census in `computeCallObligationCensus` (same file). The schedule grid's over-par memo and the Call Counts modal consume ONE census (every call-category slot: holiday-dated included, any call code, filled or not — the engine's open-slots + call-seeds count), so engine cap and UI labeling cannot disagree on what's obligatory. Calls up to the rounded obligation are never counted or labeled extra; only the provider's LAST N calls (chronological, code tiebreak; N = actual − obligation) get the grid/modal OVER treatment. Category-level fairness (fractional `bucketTarget` + deficits, §4) is untouched — rounding defines accounting and the obligatory-mode cap, never fill order.
...
**`fillMode: 'weekend-only'`** (2026-07-21, staged weekend fill — Gabriel: "just fill in the weekend call schedule … then press another button to continue the rest"):
- The main loop attempts ONLY call slots whose `derived_day_type` ∈ {saturday, sunday, friday}. Holiday day types are OUT of scope (Continue handles them). Block/day chains fire normally from weekend placements and land wherever the pattern points (Monday D1, Friday D2/D4, post-call blocks) — chains are never scope-clipped, and a chain-filled out-of-scope call slot is handled, not deferred.
- Out-of-scope call slots are NOT unfilled failures: they land in `plan.awaitingContinue` (present only in this mode, even when empty) and surface on the result as `awaitingContinue: { total, byDayType }`. The UI banner says "Weekend fill complete: N placed · M slots awaiting Continue" and shows the **Continue** button, which simply runs another generation with `fillMode: 'all'` (always `'all'` — Continue finishes the whole schedule; the select stays available for manual choices).
- Skipped in weekend-only: the pre-PTO pass (weekday Thursday-targeted, §7), relief (§10), mop-up (§10.5) — relief/mop-up rank against the FULL call schedule, which doesn't exist yet — the day-shift engine (the route skips it), and the optimizer (a deliberately partial plan must not be optimized against full-schedule objectives). The Continue run's optimizer sees the committed weekend placements as SEEDS — immovable — so reviewed weekends stay locked, which is the feature's intent. Spans still run (structural obligations like chains; no shipped pattern has one).
- Quota/scoring semantics are IDENTICAL to `'all'` (relaxation enabled, no obligation caps — modes never compose in v1).
- **Staged-equivalence property** (pinned in `weekendOnlyMode.test.ts`): because dayTypeFillOrder runs weekend groups first and `seedSolveState` reconstructs bucket counts / call dates / post-call blocks from seeds, the GREEDY portion of staged (weekend-only, commit, then Continue-`'all'`) equals the one-shot `'all'` greedy — stage-1 placements are byte-identical to the one-shot weekend subset, and the staged union equals the one-shot plan (verified on the golden fixture with AND without PTO). The ONE characterized greedy divergence is pre-PTO **pass ordering**: one-shot places the pre-PTO Thursday BEFORE the weekend groups; staged by construction places it after. When the PTO-bound provider would win a Friday call that the Thursday's post-call block would have denied them, the flows swap providers on the contended slots (staged favors the weekend placement — the reviewed weekend is authoritative) — and the swap can CASCADE through chain links and `unlessCallWithinDays` waivers, so WHICH chained day slots end up filled can differ between the flows: the staged union can cover fewer (or different) chained D fills than one-shot. Nothing disappears silently — chain suppressions are recorded in `plan.skippedDerived` (invariant 4) and, in production shape (`ctx.shiftTypes` present), every still-open chain slot is reported by the stage-2 mop-up as a sequence orphan (`chain link severed` / `pre-call fill waived`, §10.5). Pinned with a concrete cascade counterexample in `weekendOnlyMode.test.ts` (golden fixture + two live no-call requests: staged loses four chained D fills, gains two different ones, all reported; removing only the PTO makes staged byte-identical again — the cascade's sole source is the pre-PTO pass ordering). Optimizer outcomes legitimately differ (staged locks weekends) and are not pinned.
...
**Completeness check (work-to-required, 2026-07-24 — Gabriel: "make sure everyone is working the maximum number of obligatory work days"):** every row with `credited < required` additionally carries `shortfall: {days, engineGapDates, noSlotDates}` — the provider's idle working days (no credit, not netting-PTO), classified against the plan's reconstructed END state with the engine's own eligibility gate: `engineGapDates` = an open engine-addressable slot the provider was eligible for remained that day ("under-scheduled: engine gap" — open call slots checked `'call-no-quota'`; open non-sequence-owned call-engine day slots checked `'derived'`); `noSlotDates` = no open compatible slot existed ("no open compatible slots — staffing reality"). Sequence-owned slots are never addressable inventory (chain-owned, reported as orphans by §10.5); day-pool slots are the other engine's accounting. Never silent, never conflated — partials may list more idle days than `days` (their `entitledOff` idles by design); `days` is the true deficit. Rendered in the generation banner per under-required provider.
...
**Seed eviction (2026-07-21, the Hussain 9/30 bug):** multi-pass generation froze stale derived pre-fills — run 1's 10/1 C2 pre-filled 9/30 D3; run 2 filled the still-open 9/29 C2 with the same provider and its `+1 D1` link found him "already assigned" on 9/30 (his own stale auto-generated D3 seed), stranding the D1. Gabriel's standing rule (2026-07-19, the Jones fix): the post-call fill OVERRIDES any pre-call status. The engine now mirrors `sequenceAutoFill`'s row eviction for SEEDS: a **positive-offset** dayChain link fill blocked ONLY by the provider's own seeded assignment on the target date evicts the seed in-plan when it passes the gates — auto_generated only (never manual/manually_overridden), same schedule version only, a pattern **pre-fill code** (negative-offset link codes from the doc), and outranked-or-tied (`shiftRank`: realized-call trigger beats derived pre-fill, ties to post-call). The **decision predicate is single-homed** in [`preFillEviction.ts`](src/lib/rulesEngine/preFillEviction.ts) (`mayEvictPreFill` + `preFillCodes` + `shiftRank`) and consumed by BOTH `sequenceAutoFill` and the engine (`solveKernel.tryEvictSeedAndFill`) — never two divergent copies. Evictions are recorded on `plan.evictions` (never silent) and EXECUTED by `commitPlan` — the seed row reverts to open (provider null, status `'open'`, source `'manual'`, mirroring `revertToOpen`) BEFORE the fill writes; a revert that doesn't land WITHHOLDS the dependent fill (no double-booking) and reports it. The vacated pre-fill slot has no valid person (its designated person is consumed by the post-call override): it stays **OPEN** — sequence ownership (§10.5) keeps it out of mop-up/relief inventory — and the eviction record is its report. Workday credit nets to zero (the fill re-credits the same date; the ledger is a per-(provider,date) set) and bucket counts are untouched (pre-fills are non-call by gate). Pinned in `seedEviction.test.ts`; seed-free plans are byte-identical (`plan.evictions` is lazily materialized — the fill-mode golden pins hold).
...
Code: [`passes/relief.ts`](src/lib/rulesEngine/passes/relief.ts). Data: `doc.reliefPass` (`{enabled, dayTypes}`) + relief codes from `shift_types.relief_rank` ordering (legacy D4–D9 fallback when `ctx.shiftTypes` is absent). For each schedule date with any relief-code slot on an in-scope day type, providers are ranked "first on out-list": distance to their next call (any call-category code, soonest first), then that call's `call_rank` tier, then most-recently-called, then id. Per code, the scan restarts from rank 0, skipping providers already placed that date or ineligible for that specific slot — a provider skipped for D5 (excluded shift type) is still considered for D6. The per-code eligibility check is the ONLY gate (work-to-required root-cause fix, 2026-07-24): the pass formerly pre-filtered the date's whole candidate pool against ONE sample slot's eligibility, so a provider credential-excluded from the sample code was dropped for every code that date — idling under-required providers while later-code inventory sat open.

**Work-to-required ranking (2026-07-24):** `required` working days (§4.6) are a target to REACH, not only a ceiling. `rankByNextCall` (solveKernel.ts) has two modes keyed on the slot's out-position: the FIRST existing relief slot on a date is the **early-out** position — its holder leaves earliest, so next-call adjacency is the clinical point of the code and the full clinical tuple stays primary (the workday DEFICIT, `required − credited`, only breaks exact clinical ties). Every LATER relief position — and every mop-up orphan (§10.5) — is **inventory**: deficit (descending) is the primary tier so under-required providers keep getting placed until they reach `required` or the compatible inventory exhausts; the clinical tuple is preserved WITHIN an equal-deficit tier. The workdays CAP is unchanged (target ≤ credited ≤ cap — the `'derived'` gate still refuses at `required`). Without a `workDayBudget` every deficit is 0 and both modes reduce to the pre-change clinical ordering byte for byte (golden parity / fillAll pins are untouched). **Sequence-owned relief-code slots are NOT relief inventory** (2026-07-17, see §10.5): a relief-code slot the active pattern could target as a chain link (e.g. weekend-v2's Friday D4, the Sat-C3 block-chain link) is skipped outright — if its chain fired it's already handled; if the chain broke it must stay open for the mop-up sweep to report. Un-fillable relief slots land in `unfilled` with reason `'No eligible relief provider'`.

## 10.5. Mop-up sweep (orphaned call-engine day slots)

Code: [`passes/mopUp.ts`](src/lib/rulesEngine/passes/mopUp.ts). After relief (2026-07-16): any still-open NON-call slot whose shift type has `generation_engine = 'call'` (the D-chain and relief codes) is an orphan — its trigger call went unfilled or its chain severed, and the day-pool engine deliberately skips call-owned slots, so it used to vanish from every report. The sweep fills each one via the `'derived'` gate (every safety gate; no quota) using the relief-style ranking in its **inventory** mode (deficit-first, §10 — orphans are pure coverage inventory; source `'day-mop-up'`), and reports anything still open in `unfilled` (`'No eligible provider for call-engine day slot'`; slots the relief pass already reported are not double-reported). `skippedDerived` records stay untouched — the designated provider's suppression remains observable even when another provider covers the slot (invariant 4). Requires `ctx.shiftTypes` (engine-ownership is a patch18 column); without it the sweep is skipped.
...
0. **Request sort tiers** (SOFT — never a gate in either direction; a request only reorders candidates, every safety gate / quota / workdays cap / provider cap / fill-mode rule still applies):
   - **No-call avoid** (2026-07-17): candidates with a live `no_call_request` covering the slot date (`isActiveNoCallRequest`, shared.ts — pending counts, denied/canceled don't) sort behind every neutral candidate, so a requester is chosen only when nobody without a request passes the gates. Among penalized candidates, the fewest requests already violated this run wins (**fair denial** — the counter includes seeded calls on requested dates and every real placement path).
   - **Call-request prefer** (2026-07-22, the mirror image): candidates with a live `call_request` covering the slot date (`isActiveCallRequest`, shared.ts — same status semantics) sort BEFORE every neutral candidate, so a requester wins any call slot they already legally could take. Among preferred candidates, the fewest requests already granted this run wins (**fair grant** — `callReqGranted`, incremented by every real call placement and seeded calls on requested dates).
   - **Contradiction**: a provider with BOTH request types live on one date asked for opposite things — the date is treated as NEITHER (excluded from both tiers, both counters and both grant reports) and warned once per provider on `plan.requestWarnings` (lazily materialized; surfaced into `GenerationResult.warnings`).
   - Zero live requests ⇒ the tuple degenerates to 1–3 below, byte-identical plans (pinned against `fillAllPlan.golden.json` in both `noCallRequests.test.ts` and `callRequests.test.ts`). Applies in BOTH fill modes; in obligatory mode a violated (or granted) placement still consumes the cap. `GenerationResult.requestGrants` / `.callRequestGrants` report per provider the requested block dates split into granted/violated (no-call) and granted/not_granted (call), computed from the FINAL plan in [`requestGrants.ts`](src/lib/rulesEngine/requestGrants.ts); validation independently soft-flags violated no-call assignments ("No-call request") — an un-granted call request is deliberately NOT a validation flag (absence of a grant is not a violation)
...
| Change call-split weights / segment→parent mapping | `shift_types.call_burden_weight` / `parent_call_code` (patch35) + [`src/lib/callBurden.ts`](src/lib/callBurden.ts) — see §16 |
...
## 16. Call splits (2026-07-22)

Gabriel: "split calls into 2 - 12 hour shifts or even 3 8 hr shifts." Three binding decisions: (1) PER-DAY split action on the grid — site structure keeps 24h calls and auto-generation keeps placing whole calls; (2) FRACTIONAL burden — 12h segment = 0.5 call, 8h = 0.3333, a split call totals exactly ONE call across fairness/obligation/OVER; (3) POST-CALL rest goes to the OVERNIGHT segment holder only.

**Data (patch35):** `shift_types.call_burden_weight` (numeric, default 1) and `shift_types.parent_call_code` (segment → parent call code — THE grouping/caps/bucket mapping; never key logic on code-name patterns). Segment rows (Paoli C1/C2; C3 overlay excluded from v1): per parent a 2×12 pair (07-19, 19-07 crosses-midnight) and a 3×8 trio (07-15, 15-23, 23-07), all `category='call'`, `manual_only=true`, `can_auto_assign=false`, `call_rank` copied from the parent, NO `shift_templates` rows — slot creation keys on templates (the D8/D9 lesson), so segment slots exist only via the split action. The overnight segment inherits the parent's `requires_post_call_rule`; day/evening segments are false.

**Split/unsplit:** `POST /api/scheduling/schedule-slots/[id]/split { mode: '2x12'|'3x8' }` and `…/unsplit` — core in [`src/lib/scheduleSplit.ts`](src/lib/scheduleSplit.ts) (injected client, thin routes). Guards: call category, not already a segment, assignment OPEN (409 — scheduler clears first), unlocked, CURRENT schedule version, not already split; unsplit needs every sibling segment OPEN. Split creates the segment slots + open placeholder assignments FIRST and deletes the parent slot LAST (compensating rollback on failure); unsplit is the mirror. Mode selects segment types by `parent_call_code` + `duration_hours` (12 vs 8).

**Weighted burden (single home: [`src/lib/callBurden.ts`](src/lib/callBurden.ts) — `callBurdenWeight`, `parentCallCodeOf`, `formatCallWeight`, `WEIGHT_EPSILON`):** the UI census (`computeCallObligationCensus`, fteTarget.ts) counts weight sums; over-par selection picks whole assignments from the chronological end while the unpicked cumulative weight exceeds the rounded obligation (all-weights-1 ≡ the old last-N rule, byte for byte). Engine: genContext folds segments under the parent code at weight in `bucketTotals` and historical counts, and EXCLUDES `manual_only` call slots from `slotsToFill` (recorded on `ctx.manualCallSlots` — the engine never places segments but the obligation census counts them); `seedSolveState` counts segment seeds under the parent at weight (`incBucketBy`) and applies the PARENT's post-call block to an overnight segment seed whose own code has no chain data (decision 3 for the call engine); obligatory-mode and provider-cap tallies consume fractional weights parent-mapped (a C1N12 seed = 0.5 against a stated C1 cap; cap 2 with 1.0 consumed still admits a whole call, 1.5 does not). The `providerLimits` evaluator validates the same way. `WEIGHT_EPSILON` (0.01) absorbs stored-fraction noise (3 × 0.3333 = 0.9999 is not "over" a whole obligation).

**Sequence/validation:** the Weekend v2 doc (patch35, mirrored in `patterns/weekendV2.ts`) gives the C2 overnight segment codes (`C2N12`/`C2N8`) `+1 D1` dayChains on C2's day types — a manual overnight C2 segment auto-fills next-day D1 via `sequenceAutoFill`, and D1 sequence ownership follows. C1 overnight segments carry no chain data; their rest rides `requires_post_call_rule` through the existing rest guards (`sequenceAutoFill` restBlocked, `dayShiftAutoGen` post-call map, engine seed inheritance above). Open segments get the standard open-call warning (category-keyed).

**Grid:** segments render STACKED inside the parent call's row cell (`parent_call_code` lookup — no new rows; pure helpers in `gridSegments.ts`), each mini-cell independently fillable via the normal picker; Split 2×12 / 3×8 and Unsplit live in the cell popover. The Call Counts modal aggregates segments under the parent code by weight (`formatCallWeight`: one decimal only when fractional). Everything degrades gracefully pre-patch35 via narrow-retry selects (an absent column can hold no segments, so weight 1 / parent null is exact).

## 17. Scenario layer (2026-07-26, Paoli block phase 2)

A per-schedule constraint manifest that reshapes ONE generation without touching master data. Storage: `scheduling.schedules.scenario_manifest` (patch37, nullable jsonb) holds the FULL phase-1 Paoli import manifest verbatim (`src/lib/paoliBlock/manifest.ts` — the audit source of truth; the engine-facing projection is computed at load time by [`projectScenario`](src/lib/rulesEngine/scenario.ts) so projection fixes never require re-import). No manifest ⇒ `ctx.scenario` absent ⇒ the engine is **byte-identical** to the pre-scenario code (fill-mode golden pins + goldenParity).

- **Targets** (per workbook DOW bucket MTH/FRI/SAT/SUN × code, exact fractions): (a) HARD per-(bucket,code) ceilings at `ceil(target)` flowing through the GENERALIZED providerCaps key vocabulary (`providerCaps.ts` — `'C1'` per-code, `'S:MTH|C1'` bucket, `'S:NEURO_FSS'` weekend-unit, `'S:EO#i'` either-or group; one tally, one admission rule `capAdmitsPlacements`, whole-block per (bucket,code) at anchors); (b) the steering targets — `ctx.bucketTarget` override (`applyScenarioBucketTargets`, exact/unfloored) + scoreCall's target-progress ratio (placed/target — least variance). NEURO F/S/S counts **weekend units** (a Sat+Sun chain pair = ONE unit). Split segments count at `call_burden_weight` under the parent code.
- **Prohibitions** (date-specific per-code, date-specific all-call, recurring weekday+code) gate `evaluateEligibility` for CALL-category slots on **every** gate set (reason `'scenario-prohibited'`) — chains, relaxation and optimizer pins can never land on one; `preferScenarioChainClean` steers anchors away from candidates whose designed links would sever. Seeded fixed assignments stand (mandatory-retained) and validation SOFT-flags them (`scenarioProhibition` evaluator + `loadScenarioValidationCtx`, both validation paths).
- **Linkages**: hard same-weekend/same-date pairs gate placement (`'scenario-linkage'` — a member realized in another weekend refuses the mismatched placement); either-or (`SAT:ANY`/`SUN:ANY`, neuro excluded) is a group cap of `max(1, ceil(Σ member targets))` that supersedes its members' individual zeros; split-12h is executed via seeds (phase 3) and reported. Preferences are SOFT (a scoreCall tier below the request tiers + the spacing score's last key).
- **Scenario FTE** overrides the provider projection for the generation only (genContext 6c; master never written; mismatches → `ctx.warnings`). Workday budgets use the scenario FTE.
- **Spacing objective** ([`spacingScore.ts`](src/lib/rulesEngine/spacingScore.ts)): call BLOCKS = maximal chain/linkage-joined runs (inside-block adjacency never penalized); 9-key lexicographic tuple [target variance, workday shortfall, −min gap, gaps ≤ 2, consecutive weekends, gap variance, tertile spread, rolling-7 multi-block, soft-pref violations]. [`multiStart.ts`](src/lib/rulesEngine/multiStart.ts) runs greedy+optimizer K times (default 8) rotating ONLY scoreCall's final id tiebreak via a seeded FNV hash (seed 0 = identity, pinned) and keeps the best tuple. `autoGenerate` takes this path when `ctx.scenario` exists and fillMode is `'all'` (`wallClockMs` is per-start); the result gains `scenarioReport` ([`scenarioReport.ts`](src/lib/rulesEngine/scenarioReport.ts) — target vs placed, prohibition violations, linkage satisfaction, spacing stats, chosen seed + all start scores).
