...
vi.mock('@/lib/rulesEngine/autoGenerate', async (importOriginal) => ({
  ...
  autoGenerate: async (_sb: unknown, _ver: unknown, options: unknown) => {
    holder.callOrder.push('generate');
    holder.genOptions.push(options);
    return holder.genResult;
  },
}));
vi.mock('@/lib/scheduleAssistant/snapshot', () => ({
  takeSnapshot: async (
    _sb: unknown, scheduleId: string, versionId: string | null, summary: string,
  ) => {
    holder.callOrder.push('snapshot');
    if (holder.snapshotThrows) throw new Error('snapshot failed');
    holder.snapshots.push({ scheduleId, versionId, summary });
    return 'snap-1';
  },
}));
...
// ── undo point (Gabriel 2026-07-30) ─────────────────────────────────────────
describe('generation undo point', () => {
  it('snapshots BEFORE generating and returns the id', async () => {
    const { json } = await post({ fillMode: 'all' });
    expect(holder.snapshots).toHaveLength(1);
    // Order matters: a snapshot taken after the engine wrote would capture the
    // post-generation state and "undo" would be a no-op.
    expect(holder.callOrder).toEqual(['snapshot', 'generate']);
    expect(json.undoActionId).toBe('snap-1');
  });

  it('names the run so the action is identifiable', async () => {
    await post({ fillMode: 'obligatory' });
    expect(holder.snapshots[0]).toMatchObject({ summary: 'Before auto-generation (obligatory)' });
    holder.snapshots = [];
    await post({ providerIds: ['havildar'] });
    expect(holder.snapshots[0]).toMatchObject({ summary: 'Before targeted generation (obligatory)' });
  });

  it('a FAILED snapshot does not block the run — undoActionId is null', async () => {
    // Best-effort by design: a confirmed, additive generation must not be
    // refused because a convenience snapshot failed. Null ⇒ the banner says so
    // rather than offering a button that does nothing.
    holder.snapshotThrows = true;
    const { res, json } = await post({ fillMode: 'all' });
    expect(res.status).toBe(200);
    expect(json.filled).toBe(3);
