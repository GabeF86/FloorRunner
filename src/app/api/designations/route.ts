...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await sb().from('daily_designations').select('*').eq('board_date', date);
...
export async function POST(req: NextRequest) {
  ...
  const missing = missingFields(body, ['staff_id', 'designation']);
  ...
  const { data, error } = await sb()
    ...
    .upsert({ staff_id: body.staff_id as string, board_date: date, designation: body.designation as string }, { onConflict: 'staff_id,board_date' })
    .select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error }  = await sb().from('daily_designations').delete().eq('staff_id', staff_id).eq('board_date', board_date);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
