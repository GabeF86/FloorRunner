...
export interface SitesClientProps {
  /** Read by the server in the same request as the page shell, shift-type
   *  counts already folded in. */
  initialSites: Site[];
  orgId: string;
  /** Set when the server's organizations or sites read failed — shown INSTEAD
   *  of the list, because an empty list here offers to add sites that already
   *  exist. */
  loadError: string | null;
  /** Set when only the shift-type read failed: counts are unknown, but the
   *  site list itself is intact and still worth showing. */
  shiftTypesLoadError: string | null;
}

export default function SitesClient(
  { initialSites, orgId, loadError, shiftTypesLoadError }: SitesClientProps,
) {
  ...
  const [shiftTypesError, setShiftTypesError] = useState<string | null>(shiftTypesLoadError);
  ...
  // reloads (a site was added, or orgId changed under a server re-render).
  const seeded = useRef(true);

  // The organization used to be fetched here and the sites read gated behind it
  // (`if (!orgId) return`) — a round trip to learn an id that never changes,
  // with everything else queued behind it. The server component resolves it now
  // and passes it in, and reports its own failure through `loadError`.
  ...
  // and came back empty, which is the onboarding case below.
  if (loadError) {
    return (
      <div>
        <PageHeader title="Sites" />
