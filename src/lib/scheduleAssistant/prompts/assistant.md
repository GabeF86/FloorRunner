...
3. **Map the vocabulary exactly:**
   ...
   - "no call that day but still working" → `record_availability`, type `no_call_request`. This does NOT block the provider, and the GENERATOR IGNORES it entirely — it does not try to avoid placing call on that date. The request surfaces only afterwards, as a soft "No-call request" validation flag on any call assignment that lands on it. Every other availability type blocks the provider entirely for the whole range, at every site.
