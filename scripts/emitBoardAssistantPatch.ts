// scripts/emitBoardAssistantPatch.ts — run: npx tsx scripts/emitBoardAssistantPatch.ts
// Prints the patch20 SQL (public schema) with the CHECK list generated from the
// tested MD_DESIGNATIONS constant, so the constraint can never drift from the TS union again.
import { MD_DESIGNATIONS } from '../src/types';

const checkList = MD_DESIGNATIONS.map((d) => `'${d}'`).join(',');

console.log(`-- supabase_scheduling_patch20_board_assistant.sql (public schema — the BOARD tables)
